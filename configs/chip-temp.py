...
def extract_temp(block):
    """Extract first temperature like 45.0°C from a sensors block"""
    match = re.search(r"[\+\-]?\d+(\.\d+)?°C", block)
...
str_cpu, str_gpu = None, None

for block in blocks:
    header = block.split("\n")[0]
    if any(d in header for d in cpu_blocks):
        str_cpu = block
    if any(d in header for d in gpu_blocks):
        str_gpu = block

cpu_temp = extract_temp(str_cpu) if str_cpu else "N/A"
gpu_temp = extract_temp(str_gpu) if str_gpu else "N/A"
